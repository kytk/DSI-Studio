...
public:
    ...
    void learn_state(observed_iterator_type observed_from,
                     ...
                     unsigned int observation_dimension,
    ...
    {
        unsigned int hidden_dimension = observed_to-observed_from;
        std::vector<std::vector<double> > observed_le(hidden_dimension);
        for (unsigned int t = 0;t < hidden_dimension;++t)
        {
            observed_le[t].resize(hidden_value_space);
            for (unsigned int i = 0;i < hidden_value_space;++i)
        ...
        for (unsigned int t = 0;t < hidden_dimension;++t)
        {
            ...
            for (unsigned int i = 0;i < hidden_value_space;++i)
            {
                ...
                if (t == 0)
                    ...
                    for (unsigned int j = 0;j < hidden_value_space;++j)
...
public:
    ...
    void online_add_observation(observed_iterator_type observed_from,bool first_observation,unsigned int lag)
    {
    ...
    classification_type online_predict(unsigned int index) const
    {
