...
private:
    ...
    void precompute_parameters(void)
    {
        ...
        id.resize(dim);
		math::matrix_eigen_decomposition_sym(covariance.begin(),iV.begin(),id.begin(),math::dyndim(dim,dim));
        constant = 1.0;
...
public:
    ...
    double operator()(attributes_iterator_type attributes)
    {
        ...
        math::matrix_vector_product(iV.begin(),dx.begin(),idx.begin(),math::dyndim(dim,dim));
        double sum = 0.0;
        for (unsigned int i = 0;i < id.size();++i)
...
public:
                    ...
                    classifications_iterator_type classifications_from)
    {
        ...
        dyndim variance_matrix_type(attribute_dimension,attribute_dimension);
        dyndim mean_matrix_type(attribute_dimension,1);
        // initial guess
        model.resize(k);
