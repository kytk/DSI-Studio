...
public:
    ...
    {
        std::vector<unsigned int> y_count(value_space);
        unsigned int sample_size = classifications_to-classifications_from;
        ...
        for (unsigned int index = 0;index < value_space;++index)
...
public:
    ...
    void estimate(attributes_iterator_type attributes_from,
                  ...
                  unsigned char value_space)
    {
        std::vector<unsigned int> table(256*value_space);//# of X|Y
        std::vector<unsigned int> y_count(value_space);
        ...
        for (unsigned int y = 0,index = 0;y < value_space;++y)
        {
            if (y_count[y] == 0)
            {
                index += 256;
            ...
            for (unsigned int x = 0;x < 256;++x,++index)
    ...
    {
        unsigned int index = classification;
        index <<= 8;
        index += att;
...
public:
                  ...
                  unsigned char value_space)
    {
        ...
        unsigned int sample_size = attributes_to-attributes_from;
        for (unsigned int y = 0;y < value_space;++y)
        {
            double sum = 0.0;
            double sum2 = 0.0;
...
private:
    ...
    struct attribute_selector
    {
        ...
        iterator_type iter;
        unsigned int att_id;
        attribute_selector(iterator_type iter_,unsigned int att_id_):iter(iter_),att_id(att_id_) {}

        attribute_type_ operator[](unsigned int index)
        {
            return iter[index][att_id];
        }

        unsigned int operator-(const attribute_selector& rhs)
        {
...
public:
                  ...
                  unsigned char value_space)
    {
        ...
        for (unsigned int index = 0;index < le_list.size();++index)
...
public:
    ...
    classification_type predict(sample_iterator_type attributes) const
    {
        ...
        for (unsigned int index = 0;index < classification_dimension;++index)
