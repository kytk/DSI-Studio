...
struct weight_function_inverse_distance
{
    ...
    double operator()(const std::map<double,unsigned int>& neighbor_list,const std::vector<classification_type>& classification) const
    {
        double predict = 0.0;
        double sum_w = 0.0;
        std::map<double,unsigned int>::const_iterator iter = neighbor_list.begin();
        std::map<double,unsigned int>::const_iterator end = neighbor_list.end();
        for (;iter != end;++iter)
        {
            if (iter->first == 0.0)
...
public:
    nearest_neighbor(unsigned int neighbor_count_ = 3):neighbor_count(neighbor_count_) {}
    ...
    double regression(sample_iterator_type predict_attributes_) const
    {
        ...
        std::map<double,unsigned int> neighbor_list;
...
private:
    double window_size;
    unsigned int classification_dimension;
public:
    ...
    void learn(attributes_iterator_type attributes_from,
               ...
               classifications_iterator_type classifications_from)
    {
        classification_dimension = (*std::max_element(classifications_from,classifications_from+(unsigned int)(attributes_to-attributes_from))) + 1;
    ...
    double regression(sample_iterator_type predict_attributes_) const
    {
        ...
        std::map<double,unsigned int> neighbor_list;
        for (unsigned int i = 0;i < parent_type::sample_size;++i)
        {
            const std::vector<attribute_type>& Si = parent_type::attributes[i];
            double dis = 0.0;
            bool not_in = false;
            for (unsigned int index = 0;index < parent_type::attribute_dimension;++index)
            {
                double value = Si[index] - predict_attributes[index];
                value *= value;
                dis += value;
