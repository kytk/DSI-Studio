...
void watershed(const ImageType& input_image,LabelImageType& label)
{
    ...
    std::list<std::pair<pixel_index<ImageType::dimension>,unsigned int> > flooding_points;
    unsigned int basin_id = 1;
    for (typename ImageType::value_type intensity = 0; intensity < presort_table.size(); ++intensity)
    {
        ...
        typename std::list<std::pair<pixel_index<ImageType::dimension>,unsigned int> >::iterator iter = flooding_points.begin();
        typename std::list<std::pair<pixel_index<ImageType::dimension>,unsigned int> >::iterator end = flooding_points.end();
        while (iter != end)
        {
            ...
            if (label[iter->first.index()] == 0)
            {
                ...
                while(!front.empty())
                {
                    ...
                    {
                        unsigned int cur_index = neighbor_points[index].index();
